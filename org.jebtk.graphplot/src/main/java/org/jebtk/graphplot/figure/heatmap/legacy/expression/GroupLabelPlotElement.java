...
public class GroupLabelPlotElement extends ColumnMatrixPlotElement {
	 ...
	 */
	private Map<Integer, List<MatrixGroup>> mRowBlockMap = 
			new HashMap<Integer, List<MatrixGroup>>();
	...
	private void init() {
		...
		Map<Integer, List<MatrixGroup>> groupMap = 
				new HashMap<Integer, List<MatrixGroup>>();

		// keep track of the max row in use for a given indice
		Map<Integer, Integer> maxRowMap = 
				new HashMap<Integer, Integer>();

	
		for (MatrixGroup group : mGroups) {
			List<Integer> indices = 
					MatrixGroup.findColumnIndices(mMatrix, group);
