...
public class GroupLabelPlotElement extends ColumnMatrixPlotElement {
   ...
   */
  public GroupLabelPlotElement(DataFrame matrix, DoubleDim aspectRatio,
  ...
  private void drawGroups(Graphics2D g2) {
    ...
    for (int row : rows) {
      ...
      for (MatrixGroup group : mRowBlockMap.get(row)) {
        ...
        g2.drawString(name,
            x + (width - g2.getFontMetrics().stringWidth(name)) / 2,
            y);
     ...
     * 
     * g2.drawString(name, x + (width - g2.getFontMetrics().stringWidth(name)) /
     * 2, y);
     * 
     * x += width; }
     */
