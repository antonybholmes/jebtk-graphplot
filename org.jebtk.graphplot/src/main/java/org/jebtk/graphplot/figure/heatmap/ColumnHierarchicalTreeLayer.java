...
public class ColumnHierarchicalTreeLayer extends HierarchicalTreeLayer {
	...
	/*
	public void plotColorBars(Graphics2D g2,
			DrawingContext context,
			Axes axes,
			Plot plot,
			AnnotationMatrix m) {
		...
		int x = w / 2;//axes.getMargins().getLeft();
		int y = 0;
		
		
		
		int yoffset = plot.getInternalPlotSize().getH() - GroupColorBarLayer.HEIGHT * mMaxRows - GAP;
