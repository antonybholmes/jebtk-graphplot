...
public class RowHTreeLeftPlotElement extends RowMatrixPlotElement {
	...
	protected void drawTree(Graphics2D g2) {
		...
		/*
		Deque<Cluster> stack = new ArrayDeque<Cluster>();
		//Deque<Cluster> parentStack = new ArrayDeque<Cluster>();
		Deque<Integer> offsetStack = new ArrayDeque<Integer>();
		...
		while (stack.size() > 0) {
			...
			//System.err.println(cluster.getId() + " child " + c1.getId() + " " + c2.getId());
			
			int height1 = c1.getCumulativeChildCount() * blockSize.height;
			int height2 = c2.getCumulativeChildCount() * blockSize.height;
			...
			stack.push(c2);
			offsetStack.push(offset2);
			
			stack.push(c1);
			offsetStack.push(offset1);
		}
		*/
	}
}
