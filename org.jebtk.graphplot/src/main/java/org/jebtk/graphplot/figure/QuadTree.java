...
public class QuadTree<T> extends IdObject implements Iterable<QuadValue<T>> {
  ...
  public QuadTree<T> getClosest(Point p) {
    ...
    while (!stop) {
      ...
      /*
       * // We stop unless there is a sub quad nearer to // our point of
       * interest
       * 
       * stop = true;
       * 
      ...
      if (p.p.getY() <= tree.mMidPoint.getY()) {
        ...
        if (tree.northEast == null) {
          Rectangle2D bounds = new Rectangle2D.Double(
              tree.mBounds.getX() + tree.mBounds.getWidth() / 2,
              tree.mBounds.getY(), w, h);
  ...
  public static void main(String[] args) {
    QuadTree<String> closest = new QuadTree<String>(
    ...
    System.err
        .println("closest " + closest.getClosestPoint(new Point(85, 1)).p);
  }
