...
public class QuadTree<T> extends IdObject implements Iterable<QuadValue<T>> {
	...
	public QuadValue<T> getClosestPoint(Point p) {
		...
		//Rectangle2D bounds = new Rectangle2D.Double(p.x - radius, p.y - radius, radius * 2, radius * 2);
		...
		//points.add(new Point2D.Double(bounds.getX() + bounds.getWidth(), bounds.getY()));
		//points.add(new Point2D.Double(bounds.getX() + bounds.getWidth(), bounds.getY() + bounds.getHeight()));
		//points.add(new Point2D.Double(bounds.getX(), bounds.getY() + bounds.getHeight()));
	...
	public QuadTree<T> getClosest(Point p) {
		...
		while (!stop) {
			...
			/*
			// We stop unless there is a sub quad nearer to
			...
			if (ret.southEast != null) {
				d = p.distance(ret.southEast.mMidPoint);
			...
			if (ret.southWest != null) {
				d = p.distance(ret.southWest.mMidPoint);
	...
	public void add(QuadValue<T> point) {
		...
		while (!treeStack.isEmpty()) {
			...
			if (s.mPoints.size() == 2) {
				//System.err.println("aha2 " + p + " " + s.mBounds + " " + s.mPoints.toString());
		...
		if (p.p.getX() <= tree.mMidPoint.getX()) {
			...
			if (p.p.getY() <= tree.mMidPoint.getY()) {
				...
				if (tree.northWest == null) {
					Rectangle2D bounds = 
							new Rectangle2D.Double(tree.mBounds.getX(), tree.mBounds.getY(), w, h);
				...
				if (tree.southWest == null) {
					Rectangle2D bounds = 
							new Rectangle2D.Double(tree.mBounds.getX(), tree.mBounds.getY() + tree.mBounds.getHeight() / 2, w, h);
			...
			if (p.p.getY() <= tree.mMidPoint.getY()) {
				...
				if (tree.northEast == null) {
					Rectangle2D bounds = 
							new Rectangle2D.Double(tree.mBounds.getX() + tree.mBounds.getWidth() / 2, tree.mBounds.getY(), w, h);
				...
				if (tree.southEast == null) {
					Rectangle2D bounds = 
							new Rectangle2D.Double(tree.mBounds.getX() + tree.mBounds.getWidth() / 2, tree.mBounds.getY() + tree.mBounds.getHeight() / 2, w, h);
