...
public class AxisLabelLayerY2 extends PlotLayer {
  ...
  public void plotLayer(Graphics2D g2,
    ...
    drawTitle(g2,
        axes,
        axis,
        axes.getPreferredSize().width - g2.getFontMetrics().getAscent()
   ...
   * @param x the x
   */
  private static void drawLabels(Graphics2D g2,
      Axis axis,
  ...
  public static void drawTitle(Graphics2D g2, Axes axes, Axis axis, int x) {
    ...
    if (axis.getTitle().getFontStyle().getVisible()) {
      ...
      try {
        g2Temp.translate(x,
            axes.getInternalSize().getH() - (axes.getInternalSize().getH()
                - g2.getFontMetrics().stringWidth(axis.getTitle().getText()))
                / 2);
