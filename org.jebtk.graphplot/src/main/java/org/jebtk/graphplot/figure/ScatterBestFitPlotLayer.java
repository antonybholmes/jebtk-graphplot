...
SubFigure subFigure,
		...
		System.err.println("C"  + columns + " " + Arrays.toString(knotsX) + " " + axes.getX1Axis().getMin() + " " + axes.getX1Axis().getMax());
		...
		for (int i = 0; i < n - 1; ++i) {
			...
			path.curveTo(axes.toPlotX1(px[i]), 
					axes.toPlotY1(py[i]), 
					axes.toPlotX1(px[p2Index + i]), 
					axes.toPlotY1(py[p2Index + i]), 
					axes.toPlotX1(p2.getX()), 
					axes.toPlotY1(p2.getY()));
		}
