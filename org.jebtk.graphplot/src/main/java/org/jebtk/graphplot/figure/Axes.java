...
=======
   ...
   * The legend.
   */
  private LegendProperties mLegend = new LegendProperties();
  ...
  /**
   * Returns the layer model to control what is displayed on the plot and in
   * what order.
   *
   * @return the axes layer z model
   */
   ...
   */
  public boolean withinXY1Bounds(XYPoint point) {
    return mX1Axis.withinBounds(point.getX())
  ...
  public List<Point> toPlotX1Y1(DataFrame m, XYSeries s) {
    ...
    for (int i = 0; i < n; ++i) {
      points.add(toPlotX1Y1(m.getValue(i, columns.get(0)),
  ...
  /**
   * Converts a point from a series into the plot coordinates so it can be
   * drawn.
   *
   * @param p the p
   * @return the point
