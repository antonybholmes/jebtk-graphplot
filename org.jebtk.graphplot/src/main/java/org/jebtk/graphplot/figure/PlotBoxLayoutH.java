 ...
 */
public class PlotBoxLayoutH extends AxesLayout {
	...
	public void refresh(MovableLayer l) {
		...
		//System.err.println("refresh h box");

		//int maxW = sumWidth(GridLocation.CENTER, layerModel);

		//axes.setInternalPlotWidth(maxW);

		ZModel<MovableLayer> m = layerModel.get(GridLocation.CENTER);

		int x = l.getMargins().getLeft();

		for (int z : m) {
			MovableLayer a = m.getAtZ(z);

			if (a.getVisible()) {
				a.updateLocation(x, 0);
				//a.updatePlotHeight(h);

				x += a.getInternalPlotSize().getW();
			}
		}
	}
