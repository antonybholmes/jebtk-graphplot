...
public class FillTrapezoidPlotLayer extends FillPlotLayer {
	...
	protected GeneralPath getPath(SubFigure figure, 
		...
		for (int i = 0; i < xy.getPointCount() - 1; ++i) {
			...
			path.moveTo(xy.getPoint(i).x, yMin);
			path.lineTo(xy.getPoint(i).x, xy.getPoint(i).y);
			path.lineTo(xy.getPoint(i + 1).x, xy.getPoint(i + 1).y);
			path.lineTo(xy.getPoint(i + 1).x, yMin);
			path.closePath();
