 ...
 */
public class FigureLayoutGrid extends FigureLayout {
	 ...
	 */
	public FigureLayoutGrid(int rows, int columns) {
	 ...
	 */
	public FigureLayoutGrid(int rows, 
			int columns, 
			int paddingX, 
			int paddingY) {
		mRows = rows;
	...
	public void refresh(Figure figure) {
		...
		for (int r = 0; r < mRows; ++r) {
			...
			for (int c = 0; c < mColumns; ++c) {
				...
				if (rOffsets.containsKey(r)) {
					rOffsets.put(r, Math.max(rOffsets.get(r), h));
				} else {
					rOffsets.put(r, h);
				}
