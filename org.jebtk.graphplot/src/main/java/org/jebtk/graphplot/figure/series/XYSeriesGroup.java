...
public class XYSeriesGroup extends Group<XYSeries> implements ChangeListener {
	...
	/* (non-Javadoc)
	 * @see edu.columbia.rdf.lib.bioinformatics.plot.figure.series.Group#add(org.abh.lib.NameProperty)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see edu.columbia.rdf.lib.bioinformatics.plot.figure.series.Group#autoCreate()
	 */
	@Override
	...
	/*
	public static XlsxTableModel getExcelModel(XYSeriesGroup sc) throws IOException {
		// find the max rows
		int maxRows = 0;
		
		for (XYSeries series : sc) {
			if (series.getPointCount() > maxRows) {
				maxRows = series.getPointCount();
			}
		}
		...
		for (int i = 0; i < maxRows; ++i) {
			...
			for (int j = 0; j < sc.getCount(); ++j) {
				XYSeries series = sc.get(j);
				
				int c = j * 3;
				
				if (i < series.getPointCount()) {
					XYPoint point = series.getPoint(i);
	...
	*/
	...
	public static Map<Integer, XYSeriesGroup> arrangeGroupsByCluster(DataFrame m,
		...
		Map<Integer, XYSeriesGroup> ret = 
				new HashMap<Integer, XYSeriesGroup>();
	...
	/**
	 * Returns an ordered list of lists where each list corresponds to a group
	 * and each list contains the indices of the columns that match the group.
	 *
	 ...
	 * @return the list
	 */
	public static List<List<Integer>> findIndices(List<String> ids, XYSeriesGroup groups) {
		...
		try {
			while ((line = reader.readLine()) != null) {
				...
				for (String term : searchTerms) {
					List<Integer> indices = TextUtils.matches(matrix.getRowNames(), term);
					
					for (int row : indices) {
						if (inUse.contains(row)) {
							continue;
						}
