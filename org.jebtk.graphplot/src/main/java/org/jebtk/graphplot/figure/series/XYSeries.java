...
public class XYSeries extends MatrixGroup implements ChangeListener {
  ...
  /** The m marker style. */
  private StyleProperties mMarkerStyle = new StyleProperties();

  /**
   * The member font properties.
   */
  private FontProperties mFontProperties = new FontProperties();
   ...
   * @param color the color
   */
  public XYSeries(String name, Collection<Pattern> regexes,
   ...
   */
  public XYSeries(String name, Collection<Pattern> regexes,
      boolean caseSensitive, Color color, ShapeStyle shape) {
   ...
   */
  public StyleProperties getMarkerStyle() {
    return mMarkerStyle;
  }
  ...
  /**
   * Get the XY points of a series (assuming two columns, one for x values and
   * one for y values). If the series does not map to at least two columns null
   * will be returned. If the series maps to more than two columns, the first
   * will be considered to be x and the second y, all other colums will be
   * ignored.
   *
   * @param m the m
   * @param series the series
  ...
  public static List<DoublePos2D> getXYPoints(DataFrame m, XYSeries series) {
    ...
    for (int i = 0; i < m.getRows(); ++i) {
      points.add(new DoublePos2D(m.getValue(i, columns.get(0)),
   ...
   */
  public static List<XYSeries> loadSeries(Path file, DataFrame matrix)
      throws IOException {
    if (file == null) {
      return Collections.emptyList();
    }
   ...
   * @throws IOException Signals that an I/O exception has occurred.
   */
  public static List<XYSeries> loadSeries1(Path file, DataFrame matrix)
      throws IOException {
   ...
   * @return the XY series
   */
  public static XYSeries create(String name,
      List<String> regexes,
      Color color) {
   ...
   * @throws IOException Signals that an I/O exception has occurred.
   */
  public static List<XYSeries> loadSeries2(Path file, DataFrame matrix)
      throws IOException {
