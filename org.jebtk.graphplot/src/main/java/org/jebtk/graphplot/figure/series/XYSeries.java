...
public class XYSeries extends MatrixGroup implements ChangeListener {
	...
	/**
	 * Returns a multiplier between 0 and 1 to scale 
	 * bars by.
	 * 
	 * @return
	 */
	...
	/**
	 * Get the XY points of a series (assuming two columns, one for x values
	 * and one for y values). If the series does not map to at least two
	 * columns null will be returned. If the series maps to more than two
	 * columns, the first will be considered to be x and the second y, all
	 * other colums will be ignored.
	 *
	 * @param m the m
	 * @param series the series
		...
		try {
			while ((line = reader.readLine()) != null) {
				...
				for (String term : searchTerms) {
					System.err.println(name + " terms " + term);
					
					List<Integer> indices = 
							TextUtils.matches(matrix.getColumnNames(), term);
